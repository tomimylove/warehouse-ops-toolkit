...
Отступы и радиусы — стандартная (4px-кратная) шкала Tailwind + `--radius`
темы Nova (`0.625rem`, из него считаются `--radius-sm/md/lg/xl`) — не
подбираем произвольные значения вне этой шкалы.

Размер компонентов (`size` у `Button`, `Input`, `SidebarMenuButton` и т.д.):

- **`default` — основной размер везде.** Ничего не указываем явно, пока не
  возникла конкретная причина.
- **`sm`** — только в плотных местах: тулбары, строки таблиц, сайдбар
  (там, где элементы уже стоят рядом и `default` визуально распирает).
- **`lg`** не используем без отдельного запроса под конкретный экран (лендинг/
  пустое состояние с крупным CTA и т.п.) — не дефолт ни для одного модуля.

Правило исключает необходимость решать размер заново на каждом новом
компоненте/модуле.
